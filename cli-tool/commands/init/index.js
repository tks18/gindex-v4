...
module.exports = async () => {
  ...
  if (store.get('init.checked')) {
    const name = await prompt({
      ...
      name: 'checkAgain',
      message:
        'It Looks Like the You have Already Initialized, Do You want to Check Again (y/n)',
    });
  ...
  spinner(
    true,
    'Checking Git Status and Version',
    0,
    false,
    async (gitSpin) => {
      ...
      if (gitCheck.res) {
        ...
        spinner(
          true,
          'Checking NPM Status and Version',
          0,
          false,
          async (npmSpin) => {
            ...
            if (npmCheck.res) {
              ...
              console.log(
                '\n' +
                  ...
                  '\n' +
                  chalk.bold(
                    'NPM is Available and Version: ' + npmCheck.output,
                  ) +
                  '\n',
              );
                ...
                async (checkHerSpin) => {
                  ...
                  if (checkHeroku.res) {
                    console.log(
                      '\n' +
                        chalk.bold.green(
                          'It Looks like Heroku is Authorized and Now Starting the Deployment Process.',
                        ) +
                        '\n',
                    );
                    deploy.backend();
              ...
              console.log(
                '\n' +
                  ...
                  '\n' +
                  chalk.bold(
                    'Npm Failed with Following Error: ' + npmCheck.output,
                  ) +
                  '\n',
              );
              process.exit();
