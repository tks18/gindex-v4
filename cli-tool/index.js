...
yargs(hideBin(process.argv))
  .command(
    'init',
    'Initialize the Deployer (Will Check for all the Preqrequisites)',
    {},
    (args) => {
      init();
    },
  ...
  .command(
    'deploy [type]',
    'Deploy Your Backend(for Now) will Support Frontend after Some time',
    {},
    (args) => {
  ...
  .command('configure [env]', 'Update Your Backend Variables', {}, (args) => {
    if (args.env) {
      ...
      console.log(
        'More Options Will be Supported later. For Now only Backend Variables Configuration is Enabled',
      );
    }
  }).argv;
