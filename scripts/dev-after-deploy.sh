...
rm -rf vuejs/outputs/*
git clone --single-branch --depth 3 --quiet -b frontend-dev https://tks18:${GITHUB_TOKEN}@${GIT_URL} ${FT_DEPLOY_DIR}
cd ${FT_DEPLOY_DIR}
rm -rf *
cp -r ../vuejs/. .
git add .
git commit -m "Deploying Frontend for ${TRAVIS_COMMIT}-${TRAVIS_COMMIT_MESSAGE}"
git push --quiet -u --no-progress origin frontend-dev

cd ..
rm -rf ${FT_DEPLOY_DIR}/*

printf "\nPublishing Backend\n"
git clone --single-branch --depth 3 --quiet -b backend-dev https://tks18:${GITHUB_TOKEN}@${GIT_URL} ${BE_DEPLOY_DIR}
cd ${BE_DEPLOY_DIR}
rm -rf *
cp -r ../backend/. .
git add .
git commit -m "Deploying Backend for ${TRAVIS_COMMIT}-${TRAVIS_COMMIT_MESSAGE}"
git push --quiet -u --no-progress origin backend-dev

cd ..
rm -rf ${BE_DEPLOY_DIR}/*

printf "\nPublishing CLI Tool\n"
git clone --single-branch --depth 3 --quiet -b cli-tool-dev https://tks18:${GITHUB_TOKEN}@${GIT_URL} ${CLI_DEPLOY_DIR}
cd ${CLI_DEPLOY_DIR}
rm -rf *
cp -r ../cli-tool/. .
git add .
git commit -m "Deploying CLI-Tool for ${TRAVIS_COMMIT}-${TRAVIS_COMMIT_MESSAGE}"
git push --quiet -u --no-progress origin cli-tool-dev
