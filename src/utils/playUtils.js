...
export const srt2vtt = (s) => {
  const convertedFile = `WEBVTT FILE\r\n\r\n${s
    .replace(/\{\\([ibu])\}/g, '</$1>')
    .replace(/\{\\([ibu])1\}/g, '<$1>')
    .replace(/\{([ibu])\}/g, '<$1>')
    .replace(/\{\/([ibu])\}/g, '</$1>')
...
export function players(url, title) {
  const playerList = [
    ...
    },
    {
      ...
      scheme: `intent:${url}#Intent;package=com.mxtech.videoplayer.ad;S.title=${title};end`,
    },
    {
      name: 'MXPlayer(Pro)',
      icon: cdnpath('images/player/mxplayer.png'),
      scheme: `intent:${url}#Intent;package=com.mxtech.videoplayer.pro;S.title=${title};end`,
    },
  ];
  return playerList;
}
